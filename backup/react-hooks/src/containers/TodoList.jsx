...
export default function TodoList(props) {
  ...
  const deleteTodo = (num) => {
    urb.poke({app: 'todoreact', mark: 'todoreact-action', json: {'remove-task': num}})
  }

  const addTodo = (task) => {
    urb.poke({app: 'todoreact', mark: 'todoreact-action', json: {'add-task': task}})
  }
  const pokeurb = event => {
    urb.poke({app: 'todoreact', mark: 'todoreact-action', json: {'remove-task': {'num': parseInt('123'), 'label': 'abc'}}});
  }
