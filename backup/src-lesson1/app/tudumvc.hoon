...
++  on-init
  ...
  =/  todo-react  [%file-server-action !>([%serve-dir /'~tudumvc' /app/tudumvc %.y %.n])]
  =.  state  [%0 `tasks:tudumvc`~]
  :_  this
...
++  process-tasks
  ...
  ++  compare-list
  |:  [in=`[done=? id=@ud label=@tU]`[%.n 0 ''] out=`tasks:tudumvc`tasks]
